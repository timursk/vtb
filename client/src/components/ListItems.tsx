  ...
  const itemsBody = offices.map((item: any, idx: number) => (
    <div key={`${item.hours}-${idx}`} className={styles.list}>
      ...
      <div className={styles.block}>
        ...
        <span className={styles.address}>г. Москва, Красная площадь д.3</span>
        ...
        <span className={styles.metro}>
          <Image src={blueCircleIcon} alt='blueMetroIcon' /> м. Площадь
          Революции
        </span>
        <span className={styles.metro}>
          <Image src={redCircleIcon} alt='redMetroIcon' /> м. Охотный ряд
        </span>
      </div>
    </div>
  ));
